...
struct MessagesListView: View {
    ...
    @State private var chatRooms: [ChatRoomDTO] = []
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if let token = appState.accessToken {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    Text("Log in to see your conversations.")
                        .font(.system(size: 15))
                        .foregroundColor(.gray)
                        .padding(.horizontal, 16)
                    Spacer()
        ...
        .task {
            if let token = appState.accessToken {
                await loadRooms(token: token)
            }
        }
    }

    @ViewBuilder
    private func content(with token: String) -> some View {
        VStack(spacing: 0) {
            HStack {
                        ...
                        NavigationLink {
                            ...
                            HStack(spacing: 12) {
                                Circle()
                                    ...
                                    .overlay(
                                        Text(String((room.name ?? "Chat").prefix(1)))
                                            .font(.system(size: 20, weight: .bold))
                                            .foregroundColor(.white)
                                    )
                                ...
                                VStack(alignment: .leading, spacing: 4) {
                                    HStack {
                                        Text(room.name?.isEmpty == false ? room.name! : "Chat #\(room.id)")
                                            .font(.system(size: 15, weight: .semibold))
                                        Spacer()
                                    }
                                    Text(lastMessage(for: room))
                                        .font(.system(size: 13))
                                        .foregroundColor(.gray)
                                        .lineLimit(1)
                                }
    ...
    private func lastMessage(for room: ChatRoomDTO) -> String {
...
struct ChatView: View {
    ...
    var body: some View {
                    ...
                    ScrollView {
                        VStack(spacing: 12) {
                            ForEach(messages) { msg in
                                ChatBubble(message: msg, isMe: msg.senderEmail == appState.userEmail)
                                    .id(msg.id)
                            }
                        }
                        .padding(.horizontal, 16)
            ...
            HStack(spacing: 12) {
                ...
                Button {
                    ...
                    Image(systemName: "paperplane.fill")
                        ...
                        .foregroundColor(.white)
                        .padding(10)
                        .background(isSending ? Color.gray : Color.primaryBlue)
                        .clipShape(Circle())
                }
                .disabled(isSending)
            }
        ...
        .navigationTitle(room.name?.isEmpty == false ? room.name! : "Chat #\(room.id)")
        .navigationBarTitleDisplayMode(.inline)
        .task {
            await loadMessages()
        }
    ...
    private func sendMessage() {
        ...
        guard let token = appState.accessToken else {
            errorMessage = "Login required to send messages."
            return
        }
